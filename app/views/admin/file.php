                                ...
                                <div class="form-group col-auto mr-auto">
                                    <button type="button" class="btn btn-success" data-toggle="modal"
                                            data-target="#modal" onclick="modal_show()"><i
                                                class="fe fe-plus fe-12 mr-2"></i><span class="small">添加文章</span>
                                    </button>
                                </div>
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div class="form-group row">
                            <label class="col-sm-4 col-form-label">文章标题</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" id="file_title" placeholder="请输入文章标题">
                            </div>
                        </div>

                        <div class="form-group row">
                            <label class="col-sm-4">文章内容</label>
                            <div class="col-sm-8">
                                <textarea class="form-control" id="file_content" rows="4"
            ...
            <div class="modal-content">
                <div class="modal-body p-4">
                    <div class="text-center">
                        ...
                        <p class="mt-3">确认删除配置：<strong class="text-danger me-1" id="del_name">null</strong> ？<br>删除后不可恢复！请慎重操作
                        </p>
    ...
    <script>
        ...
        function modal_show(t = null) {
            if (null != t) {
                ...
                $("#modal_title").html("编辑扩展"), $("#extend_id").val(a.id), $("#file_title").val(a.filetitle), $("#file_content").val(a.filecontent)
            } else
                $("#extend_id").val(""), $("#file_title").val(""), $("#file_content").val(""), $("#modal_title").html("添加扩展")
        }

        function del_list(t, e) {
            $("#del_name").html(e), $("#del_id").val(t), $("#del_modal").modal("show")
        }

        $("#submit").click(function () {
            var t = {};
            return t.act = "编辑扩展" == $("#modal_title").html() ? "edit" : "add", "edit" == t.act && (t.id = $("#extend_id").val()), t.filetitle = $("#file_title").val(), t.filecontent = $("#file_content").val(), $("#submit").html('<i class="spinner-ui fe fe-rotate-cw fe-12 mr-2"></i>正在提交'), $("#submit").attr("disabled", !0), $.ajax({
                cache: !1,
                type: "POST",
                data: t,
                dataType: "json",
                success: function (e) {
                    $("#submit").html("提交"), $("#submit").attr("disabled", !1), 200 == e.code ? ("edit" == t.act ? (cocoMessage.success("更新成功", 2e3), add_list([t])) : (cocoMessage.success("添加成功", 2e3), add_list(e.data.list)), $("#modal").modal("hide")) : cocoMessage.error(e.msg, 2e3)
                },
            ...
            return $("#del_submit").html('<i class="spinner-ui fe fe-rotate-cw fe-12 mr-2"></i>正在删除'), $("#del_submit").attr("disabled", !0), $.ajax({
                ...
                dataType: "json",
                success: function (e) {
                    ($("#del_submit").html("确认删除"), $("#del_submit").attr("disabled", !1), 200 == e.code) ? ($("#list_" + t).remove(), cocoMessage.success(e.msg, 2e3), $("#del_modal").modal("hide"), $("#tbody_list").children().length < 1 && init(initAct, initPg)) : cocoMessage.error(e.msg, 2e3)
                },
                error: function (t, e, a) {
