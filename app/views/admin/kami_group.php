                                ...
                                <div class="form-group col-auto mr-auto">
                                    <button type="button" class="btn btn-danger" data-toggle="modal"
                                            data-target="#modal" onclick="modal_show()"><i
                                                class="fe fe-plus fe-12 mr-2"></i><span class="small">添加卡密组</span>
                                    </button>
                                </div>
            ...
            <div class="modal-content">
                <div class="modal-body p-4">
                    <div class="text-center">
                        ...
                        <p class="mt-3">确认删除卡密组：<strong class="text-danger me-1" id="del_name">null</strong>
                            ？<br>删除后不可恢复！请慎重操作</p>
    ...
    <script>
        ...
        function modal_show(t = null) {
            if (null != t) {
                ...
                $("#modal_title").html("编辑卡密组"), $("#modal_id").val(e.id), $("#modal_name").val(e.name), $("#modal_price").val(e.price), $("#modal_type").val(e.type), "vip" == e.type && ($("#modal_type_vip_span").val(sTotime(e.val, !1)), "long" == sTotime(e.val, !1) ? $("#modal_val").attr("readonly", !0) : $("#modal_val").attr("readonly", !1)), select_type(e.type), $("#modal_val").val(sTotime(e.val, !0))
            } else $("#modal_type_vip_span").val("d"), $("#modal_val").attr("readonly", !1), $("#modal_id").val(""), $("#modal_name").val(""), $("#modal_title").html("添加卡密组"), $("#modal_type").val("vip"), select_type("vip"), $("#modal_val").val(""), $("#modal_price").val("")
        }
        ...
        function select_type(t) {
            "vip" == t ? ($("#modal_type_fen_span").attr("hidden", !0), $("#modal_type_vip_span").attr("hidden", !1)) : ("addmc" == t ? $("#modal_type_fen_span").html("台") : $("#modal_type_fen_span").html("积分"), $("#modal_type_fen_span").attr("hidden", !1), $("#modal_type_vip_span").attr("hidden", !0))
        }

        function select_val_type(t) {
            "long" == t ? ($("#modal_val").attr("readonly", !0), $("#modal_val").val("9999999999")) : ($("#modal_val").attr("readonly", !1), 9999999999 == (t = $("#modal_val").val()) && $("#modal_val").val(""))
        }

        function valTotype(t) {
            return 9999999999 == t ? "永久" : isInteger(t / 60 / 60 / 24) ? t / 60 / 60 / 24 + "天" : isInteger(t / 60 / 60) ? t / 60 / 60 + "时" : isInteger(t / 60) ? t / 60 + "分" : t + "秒"
        }

        $("#submit").click(function () {
            var t = {};
            return t.act = "编辑卡密组" == $("#modal_title").html() ? "edit" : "add", "edit" == t.act && (t.id = $("#modal_id").val()), t.type = $("#modal_type").val(), t.name = $("#modal_name").val(), t.val = "vip" == t.type ? timeTos($("#modal_val").val(), $("#modal_type_vip_span").val()) : $("#modal_val").val(), t.price = $("#modal_price").val(), $("#submit").html('<i class="spinner-ui fe fe-rotate-cw fe-12 mr-2"></i>正在提交'), $("#submit").attr("disabled", !0), $.ajax({
                cache: !1,
                type: "POST",
                data: t,
                dataType: "json",
