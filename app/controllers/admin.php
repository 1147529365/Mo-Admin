...
class adminController extends Ue
{
    ...
    public $pageName = ['index' => '首页', 'home' => '总览', 'user' => '用户管理', 'file' => '文章管理', 'img_list' => '图片列表', 'img_group' => '图片分组', 'info' => '应用信息', 'ver' => '版本控制', 'reglog' => '注册登录', 'award' => '奖励控制', 'send' => '发信控制', 'logs' => '运行日志', 'extend' => '扩展配置', 'notice' => '通知公告', 'pay' => '支付控制', 'goods' => '商品管理', 'fen' => '积分事件', 'order' => '订单管理', 'agent_list' => '代理列表', 'agent_group' => '代理分组', 'agent_cash' => '提现管理', 'kami_list' => '卡密列表', 'kami_group' => '卡密分组'];//页面名称
    public $pageEnums = 10;
    public $appid;
    public $app;
    ...
    protected function __log($type, $state = 'y', $expandArr = [])
    {//记录日志
    ...
    {//文章配置
        if (U_POST) {
            $checkRules = [
                ...
                'fileTitle' => ['String', '1,255', '文章标题不规范', true],
                'fileContent' => ['String', '1,255', '文章内容不规范', true],
            ];
            $dataChecker = t('dataChecker', $_POST, $checkRules);
            $res = $dataChecker->check();
            if (!$res) $this->json($dataChecker->error, 201);
            ...
            if ($_POST['act'] == 'get') {//获取
                if (isset($_POST['so']) && !empty($_POST['so'])) {
                    $db = $db->where('(appid = ? or appid is null) and (filetitle LIKE ? or filecontent LIKE ?)', [$this->app['id'], '%' . $_POST['so'] . '%', '%' . $_POST['so'] . '%']);
                } else {
            ...
            if ($_POST['act'] == 'add') {//添加
                $query_res = $db->where('(appid = ? or appid is null) and filetitle = ?', [$this->app['id'], $_POST['filetitle']])->fetch();
                if ($query_res) $this->json('添加失败，存在重复文章标题', 201);

                $addid = $db->add(['filetitle' => $_POST['filetitle'], 'filecontent' => str_replace("&quot;", "\"", $_POST['filecontent']), 'up_time' => time(), 'reg_time' => time(), 'appid' => $this->app['id']], true);


                if ($addid) {
                    $this->__log('file_add');//记录日志
                    $list = $db->where('id = ?', [$addid])->fetch();
                    $this->json('添加成功', 200, ['list' => [$list]]);
                }
                $this->json('添加失败' . $db->error(), 201);
            ...
            if ($_POST['act'] == 'edit') {//编辑
                ...
                if (isset($_POST['filecontent'])) {
                    $query_res = $db->where('(appid = ? or appid is null) and filecontent = ?', [$this->app['id'], str_replace("&quot;", "\"", $_POST['filecontent'])])->fetch();
                    if ($query_res && $query_res['id'] != $edit_id) $this->json('编辑失败，存在重复文章', 201);
                }
                $update_res = $db->where('id = ?', [$edit_id])->update(['filetitle' => $_POST['filetitle'], 'filecontent' => str_replace("&quot;", "\"", $_POST['filecontent']), 'up_time' => time(), 'appid' => $this->app['id']]);


                if ($update_res) {
                    $this->__log('file_edit');//记录日志
                    if ($db->rowCount() >= 1) {
                        $this->json('更新成功', 200);
                    } else {
    ...
    {//图片列表
        if (U_POST) {
            $checkRules = [
                'act' => ['sameone', 'get,add,edit,edit_state,del,get_img', '操作类型有误'],
                'pg' => ['int', '1,11', '页面有误', ['get' => 1]],
                'id' => ['int', '1,11', '操作版本ID有误', ['edit,del' => false]],
                'img_name' => ['string', '2,64', '图片名称不规范', ['add,edit' => false]],
                'img_group' => ['string', '1,64', '分组名称不规范', ['add,edit' => false]],
                'img_pgid' => ['int', '1,11', '分组索引不规范', ['add,edit' => false]],
                'img_off_msg' => ['string', '2,255', '图片关闭通知内容不规范', ['add,edit' => '当前图片维护中']],
                'img_state' => ['sameone', 'off,on', '数据加密状态操作有误', ['add' => 'off', 'edit' => false, 'edit_state' => true]],
            ];
            $dataChecker = t('dataChecker', $_POST, $checkRules);
            $res = $dataChecker->check();
            if (!$res) $this->json($dataChecker->error, 201);
            ...
            if ($_POST['act'] == 'add') {//添加
                ...
                $add = [
                    ...
                    'img_group' => $_POST['img_group'],
                    'pgid' => $_POST['img_pgid'],
                    'img_off_msg' => $_POST['img_off_msg'],
                ];


                if (isset($_POST['img_url'])) {
                    // 获取 Base64 图片数据
                    $base64Image = explode(',', $_POST['img_url']);

                    // 将 Base64 图片数据解码为二进制数据
                    $imageData = base64_decode($base64Image[1]);

                    if ($imageData !== false) {
                        // 生成随机文件名，确保不重复
                        $randomFileName = uniqid() . '.jpg'; // 你可以根据需要设置文件类型

                        // 设置上传目录
                        $uploadDirectory = 'data/pic/' . $randomFileName;

                        // 将二进制数据保存为文件
                        if (file_put_contents($uploadDirectory, $imageData)) {
                            // 文件保存成功，返回图片地址
                            $add['img_url'] = '/' . $uploadDirectory;
                            // 返回成功响应
                            //$this->json('文件上传成功', 200, $add);
                        } else {
                            // 文件保存失败，返回错误消息
                            $this->json('文件保存失败', 201);
                        }
                    } else {
                        // 解码失败，返回错误消息
                        $this->json('文件解码失败', 201);
                    }
                } else {
                    // 没有上传文件或文件字段不存在，返回错误消息
                    $this->json('没有上传文件', 201);
                }
            ...
            if ($_POST['act'] == 'edit') {//编辑
                $res = $db->where('id = ? and appid = ?', [$_POST['id'], $this->app['id']])->fetch();
                if ($res && $res['img_url'] != "") {
                    unlink('.' . $res['img_url']);
                }
                ...
                if (isset($_POST['img_url'])) {
                    ...
                    if ($imageData !== false) {
                        ...
                        if (file_put_contents($uploadDirectory, $imageData)) {
                            // 文件保存成功，返回图片地址
                            $update['img_url'] = '/' . $uploadDirectory;
                            // 返回成功响应
                            //$this->json('文件上传成功', 200, $add);
                        } else {
                            // 文件保存失败，返回错误消息
            ...
            if ($_POST['act'] == 'edit_state') {//编辑
                ...
                if ($update_res) {
                    $this->__log('img_edit_state');//记录日志
                    $this->json('更新成功', 200);
                } else {
                    $this->__log('img_edit_state', 'n');//记录日志
                    $this->json('更新失败' . $db->error(), 201);
                }
            }
            ...
            if ($_POST['act'] == 'del') {//删除
                ...
                $res = $db->where('id = ?', [$_POST['id']])->delete();
                if ($res) {
                    $this->__log('img_del');//记录日志
                    $this->json('删除成功');
                } else {
                    $this->__log('img_del', 'n');//记录日志
                    $this->json('删除失败', 201);
                }
            }

            if ($_POST['act'] == 'get_img') {//获取图片组
                $pglist = db('img_group')->where('appid = ?', [$this->app['id']])->fetchAll("id,name");
                if (!$pglist) $this->json('数据获取失败', 201);
                $this->json('成功', 200, ['list' => $pglist]);
            }
    ...
    {//图片分组
        if (U_POST) {
            $checkRules = [
                ...
                'name' => ['String', '2,64', '图片组名称不规范', ['edit,add' => false]],
                'remark' => ['String', '1,9999999999', '备注不规范', ['edit,add' => false]],
            ];
            $dataChecker = t('dataChecker', $_POST, $checkRules);
            $res = $dataChecker->check();
            if (!$res) $this->json($dataChecker->error, 201);
            ...
            if ($_POST['act'] == 'get') {
                ...
                $db = $db->join("as KG LEFT JOIN (SELECT pgid,COUNT(*) AS kms,count(case when img_state='on' then 1 end) as useNum FROM {$db->pre}img_list GROUP BY pgid) AS K ON (KG.id=K.pgid)");
                ...
                $list = $db->order('KG.id desc')->page($page, $this->pageEnums)->fetchAll("KG.*,IFNULL(K.kms,0) as img_num,IFNULL(K.useNum,0) as img_uses");
                if (!$list) $this->json('数据获取失败', 201);
            ...
            if ($_POST['act'] == 'add') {//添加
                ...
                if ($res) $this->json("图片组已存在", 201);
                $data = ['name' => $_POST['name'], 'remark' => $_POST['remark'], 'appid' => $this->app['id']];
                $addid = $db->add($data);
                if ($addid) {
                    $list = $db->where('id = ?', [$addid])->fetch();
                    $list['img_num'] = 0;
                    $list['img_uses'] = 0;
                    $this->__log('img_group_add');//记录日志
                    $this->json('添加成功', 200, ['list' => [$list]]);
                } else {
                    $this->__log('img_group_add', 'n');//记录日志
                    $this->json('添加失败：' . $db->error(), 201);
                }
            }

            if ($_POST['act'] == 'edit') {//编辑
                $data = ['name' => $_POST['name'], 'remark' => $_POST['remark']];

                $query_res = $db->where('appid = ? and name = ?', [$this->app['id'], $_POST['name']])->fetch();
                if ($query_res && $query_res['id'] != $_POST['id']) $this->json('编辑失败，重复图片组名称', 201);


                $res = $db->where('id = ?', [$_POST['id']])->update($data);
                if ($res) {
                    $list = $db->join("as KG LEFT JOIN (SELECT pgid,COUNT(*) AS kms,count(case when img_state='on' then 1 end) as useNum FROM {$db->pre}img_list GROUP BY pgid) AS K ON (KG.id=K.pgid)")->where('KG.id = ?', [$_POST['id']])->fetch('KG.*,IFNULL(K.kms,0) as img_num,IFNULL(K.useNum,0) as img_uses');
            ...
            if ($_POST['act'] == 'add') {//添加
                ...
                if ($res && $snum >= 1) {
                    ...
                    if ($_POST['out'] == 'y') {
                        ...
                        $durl = '/' . U_C . '/download?path=' . t('Rc4')->mi($route, $this->admConf['ADM_KEY']);
    ...
    {//系统设置
        if (U_POST) {
            ...
            if ($_POST['sys_cache'] == 'off') {
                if (!U_CACHE) {
                    $indexdata = str_replace("'U_CACHE',FALSE", "'U_CACHE',TRUE", $indexdata);
                }
            } else {
                if (U_CACHE) {
                    $indexdata = str_replace("'U_CACHE',TRUE", "'U_CACHE',FALSE", $indexdata);
                }
            }
            if ($_POST['sys_error'] == 'on') {
                if (!U_ERROR) {
                    $indexdata = str_replace("'U_ERROR',FALSE", "'U_ERROR',TRUE", $indexdata);
                }
            } else {
                if (U_ERROR) {
                    $indexdata = str_replace("'U_ERROR',TRUE", "'U_ERROR',FALSE", $indexdata);
                }
            }
            ...
            if ($_POST['error_uploading'] == 'on') {
                if (!U_ERROR_UPLOADING) {
                    $indexdata = str_replace("'U_ERROR_UPLOADING',FALSE", "'U_ERROR_UPLOADING',TRUE", $indexdata);
                }
            } else {
                if (U_ERROR_UPLOADING) {
                    $indexdata = str_replace("'U_ERROR_UPLOADING',TRUE", "'U_ERROR_UPLOADING',FALSE", $indexdata);
                }
            }
    ...
    public function download()
    {
        ...
        $filePath = t('Rc4')->mi($path, $this->admConf['ADM_KEY'], 1);
        if (!strstr($filePath, './app/data') || !file_exists($filePath)) {
            $this->display('404.php');
        }
        t('download')->download($filePath, basename($filePath));
    }
}