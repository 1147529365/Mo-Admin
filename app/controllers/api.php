...
class apiController extends Ue{
    ...
    public function __query(){//查询订单支付状态
        $checkRules  = [
            'token' => ['Jwt','','Token令牌有误'],
    ...
    public function __notice(){//获取通知列表
        $checkRules  = [
            'token' => ['Jwt','','Token令牌有误'],
        ];
        $dataChecker = t('dataChecker',$_POST, $checkRules);
    ...
    public function __Filelist(){//获取文件列表
        $checkRules  = [
            'token' => ['Jwt','','Token令牌有误'],
        ];
        $dataChecker = t('dataChecker',$_POST, $checkRules);
    ...
    public function __getFile(){//获取文章详情
        $checkRules  = [
            'token' => ['Jwt','','Token令牌有误'],
            'id' => ['int','1,11','文章ID有误']
        ];
    ...
    public function __setFile(){//修改文章标题和内容
        $checkRules  = [
            'token' => ['Jwt','','Token令牌有误'],
            'id' => ['int','1,11','操作ID有误'],
    ...
    public function __UpCount(){
        ...
        $Ures = $this->__TokenCheck();
        $gDB = db('counts');
        $upRes = $gDB->where('appid = ? and id = ? and user = ?',[$this->app['id'],$Ures['id'],$Ures['acctno']])->update(['count'=>$_POST['count']]);
        if(!$upRes){
            $this->__log($Ures['id'],$this->m,201);
            $this->out->e(201,"更新失败");
        }
    ...
    public function __GetNotice() { // 获取通知内容
        $checkRules = [
            'token' => ['Jwt', '', 'Token令牌有误'],
            'id' => ['int', '1,11', '操作ID有误'],
        ];
